...
it('Implicit assertions', () => {
  ...
  cy.get('.table.table-bordered.assertion-table tr')
  .eq(3).then( element => {                             //такий запис дозволяє ніби "стопнути" елемент на сторінці і перевірити попап
    expect(element).to.have.class('success');
    expect(element).to.have.attr('class');
    expect(element.attr('class')).to.eq('success');    //строга перевірка, один об'єкт має дорівнювати іншому (бути тим самим об'єктом)
    expect(element.attr('class')).to.eql('success');    //не строге порівняння, як текст
    expect(element.attr('class')).to.eqls('success');    ///не строге порівняння, як текст
    expect(element.attr('class')).to.equal('success');
    expect(element.attr('class')).to.equals('success');
  })
